...
class TriMipRF(nn.Module):
    def __init__(
        ...
        net_width: int = 128,
        planes_as_parameters:bool = True
    ) -> None:
        super().__init__()
        ...
        self.planes_as_parameters = planes_as_parameters
        if self.planes_as_parameters: # or True for saving params
            self.register_parameter(
                "planes",
                nn.Parameter(torch.zeros(3, plane_size, plane_size, feature_dim)),
            )
            torch.nn.init.uniform_(self.planes, -1e-2, 1e-2)
        else:
            self.planes = None
    ...
    def query_density(
        self, x: Tensor, planes:Tensor=None, return_feat: bool = False
    ):
        assert self.planes is not None or planes is not None

        # print("query_density", x.shape)
        selector = ((x > 0.0) & (x < 1.0)).all(dim=-1)
        enc = self.encoding(
            x.view(-1, 3),
            planes if planes is not None else self.planes
